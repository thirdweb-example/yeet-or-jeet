...
export const getHourlyTransferCount = async (chainId: number, tokenAddress: string): Promise<TransferCount[]> => {
  ...
  return Object.values<TransferCount>(data.aggregations?.[0] || {}).filter(item => typeof item === 'object' && 'date' in item && 'count' in item);
}