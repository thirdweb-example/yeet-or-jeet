import { askNebula, askPerplexity, formatQuestionsWithClaude, synthesizeResponses } from '@/lib/helpers/ai';
...
export async function GET(request: NextRequest) {
  ...
  if (!tokenAddress) {
    return NextResponse.json({ success: false, error: 'Missing token address' }, { status: 400 });
  }
  ...
  if (!nebulaAnswer && !perplexityAnswer) {
    return NextResponse.json({ error: 'Failed to get answers from AI' }, { status: 500 });
  }

  const synthesis = await synthesizeResponses(info, nebulaAnswer || '', perplexityAnswer || '');
  if (!synthesis) {
    return NextResponse.json({ error: 'Failed to synthesize responses' }, { status: 500 });
  }
